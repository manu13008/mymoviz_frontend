...
.then(data=> {
  ...
  const movies = moviesData.map((data, i) => {
    ...
    return <Movie key={i} updateLikedMovies={updateLikedMovies} isLiked={isLiked} title={data.title} overview={data.overview.length > 250 ? data.overview.slice(0,250) + '...' : data.overview } poster={'https://image.tmdb.org/t/p/w500/' + data.poster_path} voteAverage={data.voteAverage} voteCount={data.voteCount} />;
  });
